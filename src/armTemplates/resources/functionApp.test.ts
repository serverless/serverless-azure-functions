  ...
  describe("App Settings", () => {
    describe("General", () => {
      it("uses custom app insights instrumentation key", () => {
        ...
        const { appSettings } =
          resource.getTemplate(config).resources[0].properties.siteConfig;
        expect(appSettings).toEqual([
          ...
          },
          {
            ...
            value:
              "[concat('DefaultEndpointsProtocol=https;AccountName=',parameters('storageAccountName'),';AccountKey=',listKeys(resourceId('Microsoft.Storage/storageAccounts', parameters('storageAccountName')), '2016-01-01').keys[0].value)]",
          },
          {
      ...
      it("gets correct app settings - python", () => {
        ...
        expect(appSettings).toEqual([
          ...
          },
          {
            name: "APPINSIGHTS_INSTRUMENTATIONKEY",
            value:
              "[reference(concat('microsoft.insights/components/', parameters('appInsightsName'))).InstrumentationKey]",
          },
        ]);
      });
  ...
  describe("Arm Parameters", () => {
    describe("Linux", () => {
      it("gets correct parameters - node 10", () => {
        ...
        const { linuxFxVersion, functionAppNodeVersion } = params;
    ...
    describe("Windows", () => {
      it("gets correct parameters - node 10", () => {
        ...
        const params = resource.getParameters(config);
        const { functionAppNodeVersion } = params;
      ...
      function assertWindowsParams(parameters: ArmParameters) {
        const { linuxFxVersion, functionAppKind, functionAppReserved } =
          parameters;
