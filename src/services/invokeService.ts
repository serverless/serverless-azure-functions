...
export class InvokeService extends BaseService {
  ...
  private async getOptions(method: string, data?: any) {
    const functionsAdminKey = await this.getAdminKey();
    const options: any = {
  ...
  private async getAdminKey(): Promise<any> {
    const url = `https://${this.serviceName}${config.scmDomain}${config.masterKeyApiPath}`;
    const options = {
      headers: {
